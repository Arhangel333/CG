...
float calculateShadow(vec4 fragPosLightSpace, vec3 normal, vec3 lightDir) {
    ...
    // Проверяем границы
    if (projCoords.z >= 1.0 || projCoords.z <= 0.1 || projCoords.x < 0.0 || projCoords.x > 1.0 || 
    ...
    // Читаем shadow map
    float shadow = texture(shadow_sampler, vec3(projCoords.x + 0.12, projCoords.y - 0.15, projCoords.z - bias)).r; //vec3(projCoords.x + 0.1, projCoords.y - 0.15, projCoords.z + bias)); vec3(projCoords.x, projCoords.y, projCoords.z + bias));
